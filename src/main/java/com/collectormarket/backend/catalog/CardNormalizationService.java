 ...
 * doesn't resolve - including titles that clearly reference a non-base parallel - is logged to
 * {@code unmatched_listing} for later triage rather than dropped.
 */
@Component
public class CardNormalizationService {
    ...
    public Optional<UUID> normalize(String rawTitle) {
        ...
        Optional<UUID> matchedCardId = parsed.parallelDetected()
                ? Optional.empty()
