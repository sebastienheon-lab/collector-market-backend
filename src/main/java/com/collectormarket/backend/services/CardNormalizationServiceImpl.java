...
/**
 * Default {@link CardNormalizationService}. Parses the title, then attempts an exact match against
 * the seeded catalog; anything that doesn't resolve - including titles that clearly reference a
 * non-base variant (parallel, autograph, relic, etc.) - is logged to {@code unmatched_listing} for
 * later triage rather than dropped.
 */
@Component
public class CardNormalizationServiceImpl implements CardNormalizationService {
    ...
    public CardNormalizationServiceImpl(
